...
/*   Updated: 2023/05/27 21:34:53 by kalshaer         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_shell_s	*raw_lexer(t_shell_s *minishell, char *str)
{
	...
	minishell->lexer->raw_tokens = ft_calloc(sizeof(char *), (minishell->lexer->num_of_tokens + 1));
	minishell->lexer->raw_tokens[0] = NULL;
	while (str[count.i] && count.j < minishell->lexer->num_of_tokens)
	{
		minishell->lexer->raw_tokens[count.j] = ft_calloc(sizeof(char), raw_token_size(str, count.i) + 1);
		count.k = 0;
		count.trigger = 0;
		while (str[count.i] && count.j < minishell->lexer->num_of_tokens)
		{
		...
		{
			if (minishell->lexer->raw_tokens[count.j])
			{
				free(minishell->lexer->raw_tokens[count.j]);
